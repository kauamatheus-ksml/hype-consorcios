...
--
-- Host: localhost    Database: u383946504_hypeconsorcio
-- ------------------------------------------------------
...
CREATE TABLE `lead_interactions` (
  ...
  `description` text DEFAULT NULL,
  `result` enum('positive','neutral','negative','no_answer') DEFAULT NULL,
  ...
  CONSTRAINT `lead_interactions_ibfk_2` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
...
CREATE TABLE `leads` (
  ...
  `vehicle_interest` varchar(200) DEFAULT NULL,
  `has_down_payment` enum('yes','no') DEFAULT NULL,
  `down_payment_value` decimal(10,2) DEFAULT NULL,
...
--
-- Temporary table structure for view `leads_detailed`
--

DROP TABLE IF EXISTS `leads_detailed`;
/*!50001 DROP VIEW IF EXISTS `leads_detailed`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8mb4;
/*!50001 CREATE VIEW `leads_detailed` AS SELECT
 1 AS `id`,
  1 AS `name`,
  1 AS `email`,
  1 AS `phone`,
  1 AS `vehicle_interest`,
  1 AS `has_down_payment`,
  1 AS `down_payment_value`,
  1 AS `source_page`,
  1 AS `ip_address`,
  1 AS `user_agent`,
  1 AS `status`,
...
--
-- Temporary table structure for view `leads_summary`
--

DROP TABLE IF EXISTS `leads_summary`;
/*!50001 DROP VIEW IF EXISTS `leads_summary`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8mb4;
/*!50001 CREATE VIEW `leads_summary` AS SELECT
 1 AS `status`,
  1 AS `total`,
  1 AS `last_30_days`,
  1 AS `last_7_days` */;
...
CREATE TABLE `sales` (
  ...
  `commission_installments` int(11) DEFAULT 5,
  `monthly_commission` decimal(10,2) DEFAULT NULL,
  `vehicle_sold` varchar(200) DEFAULT NULL,
  ...
  KEY `idx_sales_date_month` (`sale_date`),
  CONSTRAINT `sales_ibfk_1` FOREIGN KEY (`lead_id`) REFERENCES `leads` (`id`) ON DELETE CASCADE,
...
--
-- Temporary table structure for view `sales_by_seller`
--

DROP TABLE IF EXISTS `sales_by_seller`;
/*!50001 DROP VIEW IF EXISTS `sales_by_seller`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8mb4;
/*!50001 CREATE VIEW `sales_by_seller` AS SELECT
 1 AS `seller_id`,
  1 AS `seller_name`,
  1 AS `total_sales`,
  1 AS `total_value`,
  ...
  1 AS `sales_last_30_days` */;
SET character_set_client = @saved_cs_client;

--
-- Table structure for table `seller_commission_settings`
--

DROP TABLE IF EXISTS `seller_commission_settings`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8mb4 */;
CREATE TABLE `seller_commission_settings` (
  ...
  `bonus_threshold` decimal(12,2) DEFAULT NULL,
  `is_active` tinyint(1) DEFAULT 1,
  `notes` text DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  ...
  CONSTRAINT `seller_commission_settings_ibfk_1` FOREIGN KEY (`seller_id`) REFERENCES `users` (`id`) ON DELETE CASCADE,
  CONSTRAINT `seller_commission_settings_ibfk_2` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`) ON DELETE SET NULL,
  CONSTRAINT `seller_commission_settings_ibfk_3` FOREIGN KEY (`updated_by`) REFERENCES `users` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB AUTO_INCREMENT=18 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `seller_commission_settings`
--

/*!40000 ALTER TABLE `seller_commission_settings` DISABLE KEYS */;
set autocommit=0;
...
/*!40000 ALTER TABLE `seller_commission_settings` ENABLE KEYS */;
commit;
...
--
-- Dumping routines for database 'u383946504_hypeconsorcio'
--

--
-- Final view structure for view `leads_detailed`
--
...
/*!50013 DEFINER=`u383946504_hypeconsorcio`@`127.0.0.1` SQL SECURITY DEFINER */
/*!50001 VIEW `leads_detailed` AS select `l`.`id` AS `id`,`l`.`name` AS `name`,`l`.`email` AS `email`,`l`.`phone` AS `phone`,`l`.`vehicle_interest` AS `vehicle_interest`,`l`.`has_down_payment` AS `has_down_payment`,`l`.`down_payment_value` AS `down_payment_value`,`l`.`source_page` AS `source_page`,`l`.`ip_address` AS `ip_address`,`l`.`user_agent` AS `user_agent`,`l`.`status` AS `status`,`l`.`priority` AS `priority`,`l`.`notes` AS `notes`,`l`.`assigned_to` AS `assigned_to`,`l`.`created_at` AS `created_at`,`l`.`updated_at` AS `updated_at`,`l`.`contacted_at` AS `contacted_at`,`u`.`full_name` AS `assigned_to_name`,`u`.`username` AS `assigned_to_username`,(select count(0) from `lead_interactions` `li` where `li`.`lead_id` = `l`.`id`) AS `interactions_count`,(select max(`li`.`created_at`) from `lead_interactions` `li` where `li`.`lead_id` = `l`.`id`) AS `last_interaction`,(select `s`.`id` from `sales` `s` where `s`.`lead_id` = `l`.`id` limit 1) AS `sale_id` from (`leads` `l` left join `users` `u` on(`l`.`assigned_to` = `u`.`id`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `leads_summary`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`u383946504_hypeconsorcio`@`127.0.0.1` SQL SECURITY DEFINER */
/*!50001 VIEW `leads_summary` AS select `leads`.`status` AS `status`,count(0) AS `total`,count(case when `leads`.`created_at` >= current_timestamp() - interval 30 day then 1 end) AS `last_30_days`,count(case when `leads`.`created_at` >= current_timestamp() - interval 7 day then 1 end) AS `last_7_days` from `leads` group by `leads`.`status` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;

--
-- Final view structure for view `sales_by_seller`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`u383946504_hypeconsorcio`@`127.0.0.1` SQL SECURITY DEFINER */
/*!50001 VIEW `sales_by_seller` AS select `u`.`id` AS `seller_id`,`u`.`full_name` AS `seller_name`,count(`s`.`id`) AS `total_sales`,sum(`s`.`sale_value`) AS `total_value`,avg(`s`.`sale_value`) AS `avg_sale_value`,sum(`s`.`commission_value`) AS `total_commission`,count(case when `s`.`sale_date` >= current_timestamp() - interval 30 day then 1 end) AS `sales_last_30_days` from (`users` `u` left join `sales` `s` on(`u`.`id` = `s`.`seller_id` and `s`.`status` = 'confirmed')) where `u`.`role` in ('seller','manager','admin') group by `u`.`id`,`u`.`full_name` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-01-26 13:10:45
